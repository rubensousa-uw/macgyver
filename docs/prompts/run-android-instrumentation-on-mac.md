...
Important source-state check: the migration work may be uncommitted on the originating Linux machine. Before testing, confirm `samples/CameraAccessAndroid/gradle/libs.versions.toml` pins `mwdat = "0.9.0"` and that the DAT migration files are present. If they are not, stop and ask for the current branch to be pushed or for a patch/worktree transfer; do not test an older baseline.
