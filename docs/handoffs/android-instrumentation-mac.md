...
- `InstrumentationTest` contains three current-route tests. Its rule chain prepares the glasses capture-source preference, MockDeviceKit, and runtime permissions before `MainActivity` launches; the camera cases assert the first usable mock frame, photo capture, and idempotent teardown through `StreamViewModel`.
